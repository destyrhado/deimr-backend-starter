...
test('OpenAPI exposes only mounted API endpoints with reusable schemas', () => {
  ...
  for (const schema of ['User', 'AuthResponse', 'ErrorResponse', 'PaginationResponse', 'ValidationError', 'RoleUpdateResponse']) {
  ...
  assert.equal(JSON.stringify(spec).includes('deimr-backend-starter.onrender.com'), false);
});

test('OpenAPI login examples are named by role and use non-production credentials', () => {
  const examples = spec.paths['/api/v1/auth/login'].post.requestBody.content['application/json'].examples;
  ...
  assert.equal(Object.values(examples).every((example: any) => example.value.password === 'ExamplePassword123!'), true);
});
...
test('OpenAPI users list query parameters match runtime behavior', () => {
  ...
  assert.deepEqual(names, ['page', 'limit', 'search', 'role', 'status', 'sort']);
  assert.equal(parameters.find((parameter: any) => parameter.name === 'limit').schema.maximum, 100);
  assert.deepEqual(parameters.find((parameter: any) => parameter.name === 'sort').schema.enum, [
    'name',
    '-name',
    'email',
    '-email',
  ...
  const listDataProperties = spec.components.schemas.UserListResponse.properties.data.properties;
