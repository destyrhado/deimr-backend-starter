...
const validateCommonUserUpdate = (
  ...
  if (
    ...
    (!isString(password) || password.length < 8)
  ) {
    errors.push({
      field: 'password',
  ...
  if (typeof role !== 'undefined') {
    errors.push({
      field: 'role',
      message: 'Role changes are only permitted on /api/v1/users/{id}/role.',
    });
  }
  ...
  if (typeof active !== 'undefined') {
    errors.push({
      field: 'active',
      message: 'The active field is not supported. Use status instead.',
    });
  }
  ...
  if (typeof status !== 'undefined') {
    if (!allowStatus) {
      errors.push({
        field: 'status',
        message:
          'Status changes are only permitted for administrator user updates.',
      });
    } else if (
      !Object.values(UserStatus).includes(
        String(status).toUpperCase() as UserStatus,
      )
    ) {
      errors.push({
        field: 'status',
...
export const validateRoleUpdate = (
  ...
  if (
    !role ||
    !Object.values(UserRole).includes(String(role).toUpperCase() as UserRole)
  ) {
    next(
      createHttpError(400, 'Invalid role', [
        { field: 'role', message: 'Role must be USER, ADMIN, or SUPER_ADMIN.' },
      ]),
    );
    return;
  }
